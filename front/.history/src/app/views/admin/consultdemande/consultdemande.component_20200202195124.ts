...
export class ConsultdemandeComponent implements OnInit {
  ...
  async Acceptedemande(Id,vv:DemandeCredit) {
    try {
      const result: any = await this.creditService.Acceptedemand(Id,vv);
      if (result) {
  ...
  async Refusedemande(Id,vv:DemandeCredit) {
    try {
      const result: any = await this.creditService.refusdemande(Id,vv);
      if (result) {
        alert("Demande Removed");
        this.loadDemande();
      }
